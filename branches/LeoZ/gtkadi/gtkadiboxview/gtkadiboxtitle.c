...
#include <stdarg.h>
static GtkAdiBoxTitle * GET_NEW_VARG (const char *first, ...) G_GNUC_UNUSED;
static GtkAdiBoxTitle *
GET_NEW_VARG (const char *first, ...)
{
	GtkAdiBoxTitle *ret;
	va_list ap;
	va_start (ap, first);
	ret = (GtkAdiBoxTitle *)g_object_new_valist (gtk_adi_box_title_get_type (), first, ap);
	va_end (ap);
	return ret;
}
